    ...
    <!-- <el-main style="padding:0;overflow:hidden;padding-left:20px;">
      <el-container class="right-wrap">
        <el-header style="padding: 0;" height="70px">
          <MainTitle title="函数(探索空间)"></MainTitle>
          <el-input v-model="funcFilterText" placeholder="输入函数名搜索" size="mini"></el-input>
        </el-header>
        <el-main style="padding:0;height:200px;">
          <el-scrollbar style="height:100%;">
            <ul style="min-height:200px;padding-right:15px;">
              <li class="func-item" v-for="item in funcData" @dblclick="addFunc(item)">
                <div class="func-item-tit">{{item.label}}</div>
    ...
    data() {
      return {
        ...
        cmOptions: {
          ...
          addModeClass: true,
          extraKeys: {
            // Backspace: function del(cm) {
            //   var ch = cm.getTokenAt(cm.getCursor());
            //   if (ch.type == "field" || ch.type == "keyword" || ch.type == null) {
    ...
    methods: {
      checkName(rule, value, callback) {
        ...
        if (
          ...
          if (
            this.$parent.$parent.$parent.checkIfDuplicate(value.trim(), [
              2,
              this.editFieldIdx
            ])
          ) {
      ...
      updateFieldList(acceptColType) {
        ...
        if (acceptColType == "all") this.fieldList = list;
          ...
          this.fieldList = list.filter(item => {
            return acceptColType.indexOf(item.col_type.toLocaleLowerCase()) > -1;
          });
        }
      },
      ...mapActions({
              ...
              for (let index = 0; index < fieldsArr.length; index++) {
                ...
                if (fieldRes.length == 0) {
                  ...
                  !["number", "char"].includes(
                    fieldMatch(fieldRes[0].col_type, this.fieldTypeList)
                  )
                ) {
      ...
      saveComputedField() {
        ...
        let dataSetObj1 = this.$parent.$parent.$parent;
        if (
          flag &&
          flag2 &&
          this.editFieldIdx > -1 &&
          dataSetObj1.ifConnUseCpField(this.editField)
        ) {
          //校验计算字段是否已参与关联，已参与的计算字段不能修改为多表计算字段
          flag = dataSetObj1.ifUseOneTab(this.editField, transRes.tabKey);
          if (!flag) {
            this.expErrorText = "该字段已参与关联设置，无法进行多表计算编辑！";
            this.expErrorDesc = "";
            flag = false;
          }
        }
        ...
        if (flag && flag2) {
          var fieldItem = {
            ...
            iconClr: "",
            tab_alias: "",
            is_copy: this.cpFieldInfo && this.cpFieldInfo.newRow ? "1" : "0",
            phy_tab_name: "",
            is_data_priv: 0
          };
          ...
          if (this.editFieldIdx > -1) {
            fieldItem = objDeepCopy(this.editField);
          }
          ...
          let dataSetObj2 = this.$parent.$parent.$parent;
          console.info(fieldItem);
          // 校验计算字段表达式是否重复
          if (dataSetObj2.checkCpFieldExpDup(fieldItem, this.editFieldIdx)) {
          ...
          if (dataSetObj2.curResourceTypeId === "database" || dataSetObj2.curResourceTypeId === "localdb") {
            fieldItem.tab_alias = transRes.tabKey;
            let tabInfo = this.allFieldsMap.get(transRes.tabKey)[0] || {};
            fieldItem.phy_tab_name = tabInfo.phy_tab_name || "";
            fieldItem.tab_chs_name = tabInfo.tab_chs_name || "";
            fieldItem.tab_id = tabInfo.phy_tab_name || "";
            fieldItem.is_trans_dim = 0;
          }
          ...
          this.useOneTabFlag = dataSetObj2.ifUseOneTab(
            fieldItem,
            transRes.tabKey
          );
          ...
          this.preparePreviewParam(fields)
            .then(response => {
              var dataSetObj = this.$parent.$parent.$parent;
              if (
                response.data.respResult == 1 &&
                response.data.respData &&
                response.data.respData.is_success &&
                response.data.respData.is_success == true
              ) {
                that.expErrorText = "";
                this.expErrorDesc = "";
                res.isValid = true;
                if (that.inputValidFlag) {
                  if (
                    this.editFieldIdx > -1 &&
                    dataSetObj.ifConnUseCpField(this.editField)
                  ) {
                    //如果计算字段参与关联，要更新关联关系中的信息col_id和col_id_exp
                    res["upConnInfo"] = true;
                  }
                  // 如果是复制的新增计算字段，要把信息传过去
                  if (this.cpFieldInfo.newRow) {
                    res["cpFieldInfo"] = {...this.cpFieldInfo};
                  }
                  dataSetObj.saveComptedField(res);
                }
              } else {
                res.isValid = false;
                that.expErrorText = "表达式校验不通过";
                this.expErrorDesc = "";
                if (
                  response.data.respData.err_msg != undefined &&
                  response.data.respData.err_msg != ""
                ) {
                  // that.expErrorText += ",";
                  that.expErrorDesc += response.data.respData.err_msg;
                }
              }
            })
            .catch(error => {
      ...
      preparePreviewParam(col_list, res) {
        col_list[0]["col_name"] = col_list[0].element_name;
        var dataSetObj = this.$parent.$parent.$parent;
        // var item = {};
        let catalogId = "";
        if (dataSetObj.dataSetCache && dataSetObj.dataSetCache.catalogId) {
          catalogId = dataSetObj.dataSetCache.catalogId || "";
        }
        var formattedData = dataSetObj.formatDataBeforeSave(true);
        // 数据集信息
        let entity_lists = formattedData.entity_lists;
        if (dataSetObj.curResourceTypeId === "database" || dataSetObj.curResourceTypeId === "localdb") {
          col_list.forEach((item, index) => {
            col_list[index].tab_id = col_list[index].tab_alias;
          });
        }
        //如果计算字段只涉及一张表，则实体只传自己的信息，其他表不传，用以处理新疆结果表两张表关联字段类型不一致不能造计算字段的问题
        if (this.useOneTabFlag) {
          entity_lists = entity_lists.filter(entity => {
            return entity.tab_alias == this.curUseTab;
          });
          if (entity_lists.length > 0) {
            entity_lists[0].tab_join_exp_list = [];
            entity_lists[0].rel_tab_id = "";
            if (entity_lists[0].rel_tab_alias) entity_lists[0].rel_tab_alias = "";
          }
        }
        let param = {
          anly_frame_info: {
            af_desc: dataSetObj.dataSetName,
            af_id: "",
            af_id_list: [],
            af_name: dataSetObj.dataSetName,
            af_status: "1",
            af_status_desc: "",
            af_status_list: [],
            af_type: "",
            anly_subject_id:
              dataSetObj.curAfId == "" ? "" : dataSetObj.curSubjectId,
            anly_subject_name: "",
            create_time: "",
        ...
        if (dataSetObj.curResourceTypeId === "database" || dataSetObj.curResourceTypeId === "localdb") {
          params["dgw_tables"] = dataSetObj.genDgwTables(entity_lists);
        }
      ...
      checkNumField(colFunExp) {
        ...
        for (let index = 0; index < this.allFileds.length; index++) {
          ...
          let fieldType = fieldMatch(element.col_type || "", this.fieldTypeList);
          if (
            fieldType != "number" &&
            colFunExp.indexOf(
              `[${element.tab_alias}.${
