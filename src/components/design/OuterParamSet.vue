              ...
              <div style="height:100%; padding-right:20px;">
                <div v-for="(item,index) in paramList" class="param-item-wrap">
                  <div
                    ...
                    @dblclick="beginEdit(index,item)"
                  >
                    <input v-if="editInput===index" @focus="nameCache=item.name" @blur="nameCheck(item)" type="text" class="param-input" v-model="item.name" maxlength="12">
...
export default {
  ...
  methods: {
    ...
    nameCheck(item){
      ...
      let sign = false;
      // 检测重名
      let nameList = this.paramList.map(val=>{
        return val.name
      });
      if((new Set(nameList)).size !== nameList.length){
        this.$message.error(`参数名与${name}重复`);
        sign = true;
      } else {
        // 检测命名规范
        let reg=/^[^0-9][A-Za-z0-9_]+$/;  //只允许大小写字母，下划线
        if(!reg.test(name)){
          this.$message.error(`参数不能以数字开头，只能包含字母数字和下划线`);
          sign = true;
        }
      }
      this.editInput='';
      if(sign) {
        item.name = this.nameCache;
      }
    },
    checkCurParamState(isCheckAll = false) {
      ...
      let warnTip = `外部参数${curParamInfo.name || ""}未完成设置`;
      ...
      if (!this.validSelect() || this.checkedDataset.length == 0) {
        ...
        for (let index = 0; index < this.paramList.length; index++) {
          ...
          if (
            ...
            this.$message({
              message: `外部参数${element.name || ""}未完成设置`,
              type: "warning"
            });
            flag = false;
            break;
    ...
    addParam() {
      ...
      this.editInput = this.paramList.length-1;
      setTimeout(() => {
        document.querySelector(".param-input").focus();
      }, 0);
    },
