...
export default {
  ...
  watch: {
    ...
    "vscompt.comptData": {
      async handler (newVal, oldVal) {
        ...
        this.vscompt.comptAttrs.extendSetting.series[0].symbolSize = (val) => {
          if (!val[2]) {
            return 0;
          }
          let avg = (range[0][1] - range[0][0]) / 20;
          let ret = parseInt(val[2] / avg);
          console.log(ret + ',');
          return ret;
        };
      }
  ...
  methods: {
        ...
        .then(function(response) {
          if (response.data.respResult == 1) {
            ...
            if (
              ...
              //如果显示数据标签，重置散点data格式
              if (extend.series[0].label.normal.show && _this.vscompt.type === "VeAreaMap")
    ...
    resetGeo() {
      ...
      //地图范围，给地图层级和名称赋值
      if (['VeLinesBmap'].includes(newData.type)) {
