          ...
          >
            <el-container class="dataset-right-top">
              <el-header height="22px" style="padding:16px 20px 20px;">
                <MainTitle title="实体关系拓扑" style="height:22px;" width="120px"></MainTitle>
                <el-tooltip class="item" effect="dark" content="同步表结构" placement="top-start">
                  <i
                    class="dataset-opt-icon icon-A10671_cutover"
                    @click.prevent="refreshStructure"
                  ></i>
                </el-tooltip>
              </el-header>
                ...
                <el-container style="position:relative;overflow:hidden;">
                                ...
                                >
                                  <ul style="line-height: 35px;">
                                    <li
                                      ...
                                      @click="saveDataPriv(scope.rowIndex,scope.row,item,$event)"
                                    >
                                      <i
                  ...
                  <el-main id="table2" style="padding: 0 0 0 10px;width:50%">
                    ...
                    >
                      ...
                      <vxe-table-column align="center" width="60px">
                        <template slot="header" slot-scope="scope">
                          <el-tooltip
                            class="item"
                            effect="dark"
      ...
      <el-container class="conn-pop-container">
        <el-header height="85px">
          <el-row :gutter="20" style="height:100%;" class="conn-pop-top-wrap">
            <el-col :span="(dataSrcValue.resourceTypeId || '').toLowerCase() === 'mysql' ? 8 : 6">
              <div
                class="grid-content"
            ...
            <el-col :span="6" v-if="(dataSrcValue.resourceTypeId || '').toLowerCase() !== 'mysql'">
              <div
                class="grid-content"
                :class="{'selected':activeConn=='6'}"
                @click="updateActiveConn('6')"
              >
                <span class="conn-pop-conn-icon opt">
                  <i class="active"></i>
                  <i class="active"></i>
...
export default {
  ...
  methods: {
    compareTable(){
      ...
      let changeStr = "";//字段变更提示
      for(let i = 0; i < entityDataList.length; i++){
        let typeChangeFields = [];
        let deleteFields = [];
        //同步之后的实体
        let curFields = this.allFieldsMap.get(entityDataList[i].tab_alias);
        //同步之前的旧实体
        let curOldFields = this.oldAllFieldsMap.get(entityDataList[i].tab_alias);
        //将旧实体的字段都转成map形式
        let oldFieldMap = new Map();
        curOldFields.filter(field => oldFieldMap.set(field.col_id, field));
        //遍历新实体，和旧实体比较，是否删除或者修改了字段类型
        for(let j = 0;j < curFields.length; j++){
          let field = curFields[j];
          let value = objDeepCopy(oldFieldMap.get(field.col_id));
          if(value && JSON.stringify(value) !== "{}"){
            if(value.col_type !== field.col_type){//新旧实体字段类型改变，存入改变列表
              value.col_type = field.col_type;
              typeChangeFields.push(field);
            }
            value.col_chs_name = field.col_chs_name;
            curFields[j] = value;
            //同步之后，备注和字段类型更新，其他保持旧的数据
            this.table1DataOri.concat(this.table2DataOri).forEach(item => {
              if(item.tab_id === field.tab_id && item.phy_tab_name === field.phy_tab_name && item.col_id === field.col_id){
                item.col_chs_name = field.col_chs_name;
                item.col_type = field.col_type;

                if(item.is_calcul === "0"){
                  //字段描述表更，计算字段的表达式描述也要变更
                  let oldElement = objDeepCopy(oldFieldMap.get(field.col_id));
                  let arr = this.getRelComputedField(item);
                  if(arr.length > 0){
                    arr.forEach(calculItem => {
                      let olddrsc = "["+oldElement[this.SHOW_TYPE_ARR[item.show_type]]+"]";//旧字段描述
                      let newdrsc = "["+field[this.SHOW_TYPE_ARR[item.show_type]]+"]";//新字段描述
                      calculItem.col_fun_exp_desc = calculItem.col_fun_exp_desc.replace(olddrsc,newdrsc);
                    });
                  }
                }
              }
            });
            //新旧数据匹配得上的删除，剩下是被删除的字段
            oldFieldMap.delete(field.col_id);
          }
        }
        this.allFieldsMap.set(entityDataList[i].tab_alias,curFields);
        //新实体与旧实体匹配不上的，及实体删除的字段列表
        deleteFields = [...oldFieldMap.values()];
        let typeColIds = typeChangeFields.map(item=>{
          return item.col_id;
        });
        let deleteColIds = deleteFields.map(item=>{
          return item.col_id;
        });

        if(typeColIds.length > 0 || deleteColIds.length > 0){
          changeStr = changeStr + "" + entityDataList[i].phy_tab_name + "表中：物理字段名为"
          if(typeColIds.length > 0){
            changeStr = changeStr + typeColIds.join("、") + "字段类型已改变，";
          }
          if(deleteColIds.length > 0){
            changeStr = changeStr + deleteColIds.join("、") + "字段已删除，" ;
          }
        }
      }
      if(changeStr !== ""){
        changeStr = changeStr + "请重新设计!";
        this.$message.warning(changeStr);
      }
    ...
    clrClass 图标颜色
    */
    queryTabFieldsNew(paramObj) {
      this.loading = true;
      let _this = this;
      let _param = {};
      let colListKey = "tab_col_list";
      let tabIdArr = [];
      tabIdArr = paramObj.map(item=>{
        return item.tab_id;
      });
      if (this.curResourceTypeId === "dataasset") {
        _param.tab_id_list = tabIdArr;
          ...
          if (response.status == 200 && response.data.respResult == "1") {
            ...
            this.queryTbFields({    //拖入实体
              ...
              if (
                ...
                for (let index = 0; index < paramObj.length; index++) {
                  const element = allData.filter(item => {
                    return (
                      item.phyTabName == paramObj[index].phy_tab_name
                    );
                  });
                  if (element.length > 0) {
                    ...
                    _this.formatFieldList(
                      objClone(colData, 1),
                      paramObj[index].tab_alias,
                      paramObj[index].color,
                      false
                    );
                  }
                }
    ...
    refreshStructure(){
      //同步前所有字段备份
      for (let [key, value] of this.allFieldsMap.entries()) {
        this.oldAllFieldsMap.set(key, objDeepCopy(value));
      }
      var entityDataList = this.treeJsonToArray(objDeepCopy(this.entityData));
      if(entityDataList && entityDataList.length > 0){
        // 查询所有字段信息
        this.queryTabFieldsNew(entityDataList);
      }else{
        this.$message.warning("请拖入需要同步的实体！");
      }
    },
    ...
    formatFieldList(getStructureData, key, clrClass, isAddToTbData) {
      ...
      for (let index = 0; index < data.length; index++) {
        ...
        let item = {
          ...
          is_data_priv: 's' //是否数据权限控制 number
        };
        if (
          element.dimId != null &&
          element.dimId != "" &&
    ...
    */
    ...
    showDataPrivList(type) {
      ...
      this.getUserTagPriv({}).then(response => {
        if (response.data.respResult == 1) {
          ...
          if (_this.privTagList.length > 0 && type === 1) {
            _this.dataPrivPropVis = true;
          }
        } else {
          _this.$message.error(
            response.data.respErrorDesc ||
    ...
    saveDataPriv(index, row, data) {
      if (this.privTagSetList.includes(data.index) && row['priv_tag'] !== data.index) {
        return;
      }
      let rowData;
      rowData = this.table1DataOri;
    ...
    table1DataTest() {
      ...
      for (let i = 0; i<500; i++) {
        let data = {
          ...
          "element_type": 1,
          "is_calcul": "0",
          "tab_id": "QRY_REJ_ONCE_USER_DETAIL",
          "tab_chs_name": "",
          "tab_col_id": "NAME",
          "col_type": "VARCHAR2",
          "col_fun_exp": "",
          ...
          "tab_alias": "t1",
          "is_copy": "0",
          "phy_tab_name": "QRY_REJ_ONCE_USER_DETAIL",
          "col_length": "255",
          "col_precise": null,
          "data_type": null,
          "is_data_priv": '0'
...
.dataset-right-top .dataset-opt-icon{
  color: $main-color;
}
</style>
